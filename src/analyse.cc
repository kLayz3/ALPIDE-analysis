...
auto main(int argc, char* argv[]) -> int {
	...
	if(IsCmdArg("track", argc, argv)) {
		cout << "Constructing tracks ...\n";
		string calFile; string saveFile;

		if(!ParseCmdLine("cal", calFile, argc, argv)) cout << __PRETTY_FUNCTION__ << " -> calibration file not provided, is ok.\n";
		ParseCmdLine("save", saveFile, argc, argv);
...
void TrackAnalysis(const char* fileName, const char* calibFile, ulong firstEvent=0, ulong maxEvents=0, const char* saveFile=nullptr) {
	...
	/* if no calibFile is given, calib fields are defaulted. */
	...
	for(ulong evNum = firstEvent; evNum < lastEvent; ++evNum) {
		...
		AlpideTrack::ColVCal(detV, colV); 
		AlpideTrack::RowVCal(detV, rowV);
		...
		while(detV.size()) {	
			...
			if(saveFile && strlen(saveFile)) { /* branch prediction */
			...
			hPhiY->Fill(phiY);
			hXY->Fill(x0,y0);
			if(saveFile && strlen(saveFile)) tree->Fill();
		}
	}
