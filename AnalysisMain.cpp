...
#define ALPIDE_NUM 6
// TODO : change all 6 into the preproc directive 
#define LUKE 0
...
/* Global containers */
UInt_t Row1[MAX_HITS];
UInt_t Col1[MAX_HITS];
UInt_t row1;
UInt_t col1;

UInt_t Row2[MAX_HITS];
UInt_t Col2[MAX_HITS];
UInt_t row2;
UInt_t col2;

UInt_t Row3[MAX_HITS];
UInt_t Col3[MAX_HITS];
UInt_t row3;
UInt_t col3;

UInt_t Row4[MAX_HITS];
UInt_t Col4[MAX_HITS];
UInt_t row4;
UInt_t col4;

UInt_t Row5[MAX_HITS];
UInt_t Col5[MAX_HITS];
UInt_t row5;
UInt_t col5;

UInt_t Row6[MAX_HITS];
UInt_t Col6[MAX_HITS];
UInt_t row6;
UInt_t col6;
...
vector<UInt_t*> ColHandle{nullptr, Col1, Col2, Col3, Col4, Col5, Col6};
vector<UInt_t*> RowHandle{nullptr, Row1, Row2, Row3, Row4, Row5, Row6};
// FIXME rewrite all rowi's into row[i] and Rowi[] into Row[][], same as cols.
// careful not to dereference handle[0] ... fix this
...
void SetOneBranchAddress(TTree* h101, int x) {
    ...
    h101->SetBranchAddress(Form("ALPIDE%dROWv", x),RowHandle[x]);
    h101->SetBranchAddress(Form("ALPIDE%dCOLv", x),ColHandle[x]);
    h101->SetBranchAddress(Form("ALPIDE%dROW", x),rowHandle[x]);
    h101->SetBranchAddress(Form("ALPIDE%dCOL", x),colHandle[x]);
}
...
int main(int argc, char* argv[]) {
    if(IsHelpArg(argc, argv)) {cout << helpString; return 0; }
